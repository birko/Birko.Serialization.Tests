...
{
    public class ProtobufBinarySerializerTests
    {
        private readonly ProtobufBinarySerializer _serializer = new();

        [Fact]
        public void ContentType_ReturnsProtobuf()
        {
            _serializer.ContentType.Should().Be("application/x-protobuf");
        }

        [Fact]
        public void Format_ReturnsProtobuf()
        {
            _serializer.Format.Should().Be(SerializationFormat.Protobuf);
        }

        [Fact]
        public void RoundTrip_Bytes_PreservesData()
        {
            var original = new TestPayload { Name = "protobuf", Value = 33, IsActive = true };
        ...
        [Fact]
        public void SerializeToBytes_ProducesBytes()
        {
            var payload = new TestPayload { Name = "test", Value = 1, IsActive = true };
        ...
        {
            var original = new TestPayload { Name = "typed", Value = 88, IsActive = true };
            var bytes = _serializer.SerializeToBytes(original);
